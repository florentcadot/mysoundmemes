...
import { CreateBoardSoundResponseDto }
...
import { UpdateBoardSoundResponseDto }
...
import { GetBoardSoundListResponseDto }
...
export class RealBoardRepository extends RealSharedRepository implements BoardRepository {
  async createBoard (props: CreateBoardForm): Promise<Board> {
    const response = await this.instance.post<CreateBoardResponseDto>(
      '/board',
      CreateBoardMapper.toRepository(props),
    )
  ...
  async updateBoard (props: UpdateBoardForm): Promise<Board> {
    const response = await this.instance.patch<UpdateBoardResponseDto>(
      '/board',
      UpdateBoardMapper.toRepository(props),
    )
  ...
  async createBoardSound (props: CreateBoardSoundForm): Promise<BoardSound> {
    const response = await this.instance.post<CreateBoardSoundResponseDto>(
      '/sound',
      CreateBoardSoundMapper.toRepository(props),
    )
  ...
  async getBoardSoundList (props: GetBoardSoundListForm): Promise<BoardSound[]> {
    const response = await this.instance.post<GetBoardSoundListResponseDto[]>(
      '/sound/list',
      GetBoardSoundListMapper.toRepository(props),
    )
  ...
  async updateBoardSound (props: UpdateBoardSoundForm): Promise<BoardSound> {
    const response = await this.instance.patch<UpdateBoardSoundResponseDto>(
      '/sound',
      UpdateBoardSoundMapper.toRepository(props),
    )
