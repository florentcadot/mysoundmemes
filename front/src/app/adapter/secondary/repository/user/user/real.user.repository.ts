...
export class RealUserRepository extends RealSharedRepository implements UserRepository {
  ...
  async login(props: LoginUserForm): Promise<LoginUserResponse> {
    const response  = await this.instance.post<LoginUserResponseDto>('/auth/login', LoginUserMapper.toRepository(props))
  ...
  async getUserProfile(props: GetUserProfileForm): Promise<User> {
    const response  = await this.instance.post<GetUserProfileResponseDto>('/user/profile', GetUserProfileMapper.toRepository(props))
  ...
  async updateUser(props: UpdateUserForm): Promise<User> {
    const response =  await this.instance.patch<UpdateUserResponseDto>('/user', UpdateUserMapper.toRepository(props))
    return UpdateUserMapper.toDomainResponse(response.data)
  }

  async forgetPasswordUser(props: ForgetPasswordUserForm): Promise<void> {
    await this.instance.post<void>('/user/forget-password', ForgetPasswordUserMapper.toRepository(props))
  }

  async resetUserPassword(props: ResetUserPasswordForm): Promise<void> {
    await this.instance.post<void>(`/user/password/${props.token}`, ResetUserPasswordMapper.toRepository(props))
  }
