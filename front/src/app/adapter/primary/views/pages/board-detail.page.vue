  ...
  <div>
    <div class="row justify-center">
      <div class="justify-center">
        ...
        <div v-if="boardSoundList.length > 0"
             class="board-list-content-container">

          <q-list class="q-mt-lg"
                  bordered
                  padding
                  v-for="sound in boardSoundList"
                  :key="sound.id">
    ...
    <q-dialog v-model="createSoundDialog">
      <q-card class="dialog-content">
        ...
        <q-card-section>
          <create-board-sound-form-component :boardId="$router.currentRoute.params.id"
                                             @submitForm="createSound"/>
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <q-dialog v-model="updateSoundDialog">
      <q-card class="dialog-content">
        ...
        <q-card-section>
          <update-board-sound-form-component
            :soundToUpdate="soundToUpdate"
            :boardList="boardList"
            @submitForm="updateSound"/>
        </q-card-section>
      </q-card>
    </q-dialog>
    ...
    <div class="action-button">
      <q-btn v-if="$q.screen.width < 400"
             color="accent"
             label="Add"
             icon="add"
             @click="createSoundDialog = !createSoundDialog"/>
      <q-btn v-else
             color="accent"
             label="Add sound"
             icon="add"
