    ...
    <q-header class="bg-primary text-white">
      <q-toolbar>
        ...
        <q-btn flat
          ...
          <q-avatar v-else
                    icon="account_circle"></q-avatar>

          <q-menu>
            <q-list style="min-width: 100px">
              <q-item clickable
                      v-close-popup
                      to="/setting/user-account"
                      exact>
    ...
    <div class="app-page-container">
      <div class="breadcrumbs-container">
        <q-breadcrumbs class="breadcrumbs">
          <q-breadcrumbs-el v-if="$router.currentRoute.fullPath.includes('board')"
                            label="Boards"
                            icon="developer_board"
                            to="/board" />
          <q-breadcrumbs-el v-if="$router.currentRoute.fullPath.includes('board/')"
                            :label="selectedBoardName"/>
          <q-breadcrumbs-el v-if="$router.currentRoute.fullPath.includes('setting')"
                            label="Settings"
                            icon="settings"
                            to="/setting" />
          <q-breadcrumbs-el v-if="$router.currentRoute.fullPath.includes('setting/user-account')"
                            label="User account"/>
        </q-breadcrumbs>
      </div>
